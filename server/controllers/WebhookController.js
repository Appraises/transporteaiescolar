...
class WebhookController {
  ...
  async handleEvolutionAPI(req, res) {
    try {
      ...
      // Suporte a minÃºsculas e maiÃºsculas (Evolution v1 vs v2)
      const validEvents = ['messages.upsert', 'MESSAGES_UPSERT', 'messages.create', 'MESSAGES_CREATE']; // Adicionei variaÃ§Ãµes de CREATE por precauÃ§Ã£o
      ...
      if (messages.length === 0) {
          console.log('[Webhook] âš ï¸ Nenhuma mensagem encontrada no campo data.');
          return;
      }
      ...
      // Tratamento Antigo de MÃ­dias/Imagens para Recibos
          ...
          if(hasBase64) {
            console.log(`[Financeiro] Analisando MÃ­dia Multimodal de ${remoteJid}...`);
      ...
      if (!passageiro && vanMatch) {
          ...
          EvolutionService.sendMessage(remoteJid, `ðŸ‘‹ OlÃ¡! Bem-vindo(a) ao Assistente da Van do(a) *${motoristaAlvo.nome}*!\nVamos configurar a vaga do passageiro rapidinho.\n\n*1. Qual o nome completo do aluno que irÃ¡ na Van?*`);
          return;
      }
